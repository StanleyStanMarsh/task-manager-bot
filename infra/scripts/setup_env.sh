#!/bin/bash
# =============================================================================
# скрипт подготовки окружения сервера
#
# После выполнения сервер готов к запуску приложения командой:
#
#     docker compose up
#
# Скрипт выполняет следующие действия:
#
#   1. Устанавливает базовые системные утилиты
#   2. Устанавливает Docker Engine и docker compose plugin
#   3. Устанавливает Java 23 (Temurin)
#   4. Монтирует Cinder-том:
#        /dev/vdb -> /var/lib/mongodb
#      (том используется для хранения данных MongoDB)
#   5. Создаёт пользователя botuser и добавляет его в группу docker
#   6. Создаёт директорию проекта /opt/task-manager-bot
#
# Лог работы скрипта сохраняется в:
#   /var/log/setup-docker-env.log
# =============================================================================
